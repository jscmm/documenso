...
| `NEXT_PUBLIC_SOURCE_CODE_URL`      | Link shown to signers for obtaining this instance's source code; set it to your repository when running a modified version (AGPL-3.0 §13) | `https://github.com/documenso/documenso` |
...
| `NEXT_PUBLIC_AUTO_SIGN_THRESHOLD`     | Offer the auto-sign dialog when a recipient has more than this many auto-signable fields (name, initials, email, date); `0` offers it for any such field | `5`     |
| `NEXT_PUBLIC_AUTO_INSERT_FIELD_TYPES` | Comma-separated field types (`NAME`, `EMAIL`) filled in for the recipient without a click on V2 envelopes, like `DATE` fields always are; the server inserts them at completion |         |
| `NEXT_PUBLIC_FIELD_TEXT_COLOR`        | CSS colour that inserted values are drawn in on the signing page and in the sealed PDF (labels stay black); invalid values fall back to black | `black` |
| `NEXT_PUBLIC_FIELD_TEXT_FONT_STYLE`   | Font style for those values: `normal`, `italic`, `bold` or `italic bold`                             | `normal` |
| `NEXT_PUBLIC_FIELD_TEXT_STYLE_TYPES`  | Comma-separated field types (`NAME`, `EMAIL`, `DATE`, `INITIALS`, `TEXT`, `NUMBER`) that take the colour and style | `NAME,EMAIL,TEXT,NUMBER` |
| `NEXT_PUBLIC_SIGNATURE_FRAME`         | `true` draws a bracket down the left edge of inserted signatures and initials with a small caption above ("Signed by:" / "Initial"), on the signing page and in the sealed PDF | `false` |
| `NEXT_PUBLIC_SIGNATURE_FRAME_COLOR`   | CSS colour of that bracket                                                            | `#3300ff` |
